export type CliDomain =
  | "chat"
  | "reports"
  | "projects"
  | "connections"
  | "billing"
  | "frontend"
  | "config"
  | "diagnostics"
  | "sessions"
  | "models"
  | "setup";

export interface CliCommandMetadata {
  name: string;
  description: string;
  options: string[];
  domain: CliDomain;
  workflows?: string[];
}

const authOptions = [
  "--base-url",
  "--api-key",
  "--api-key-stdin",
  "--machine",
  "--non-interactive",
];

const outputOptions = [
  "--format",
  "--output",
  "--open",
  "--print-url",
  "--no-open",
  "--frontend-url",
];

export const cliCommands: CliCommandMetadata[] = [
  {
    name: "tui",
    description: "Open the CloudEval Terminal UI",
    domain: "chat",
    options: [
      "--base-url",
      "--tab",
      "--project",
      "--frontend-url",
      "--api-key",
      "--api-key-stdin",
      "--machine",
      "--model",
      "--debug",
      "--health-check",
      "--no-banner",
      "--no-anim",
      "--verbose",
      "--profile",
      "--help",
    ],
    workflows: ["tui"],
  },
  {
    name: "chat",
    description: "Start an interactive chat session",
    domain: "chat",
    options: [
      "--base-url",
      "--api-key",
      "--api-key-stdin",
      "--machine",
      "--conversation",
      "--model",
      "--debug",
      "--health-check",
      "--no-banner",
      "--no-anim",
      "--verbose",
      "--profile",
      "--help",
    ],
    workflows: ["chat", "sessions resume"],
  },
  {
    name: "ask",
    description: "Ask a single question",
    domain: "chat",
    options: [
      ...authOptions,
      "--project",
      "--model",
      "--output",
      "--format",
      "--json",
      "--progress",
      "--quiet",
      "--no-color",
      "--open",
      "--print-url",
      "--no-open",
      "--frontend-url",
      "--debug",
      "--verbose",
      "--profile",
      "--help",
    ],
    workflows: ["ask"],
  },
  {
    name: "reports",
    description: "Access cost and Well-Architected Framework reports",
    domain: "reports",
    options: [
      "list",
      "show",
      "cost",
      "waf",
      "download",
      "run",
      "rules",
      ...authOptions,
      "--project",
      "--format",
      "--raw",
      "--parsed",
      "--formatted",
      "--kind",
      "--period",
      "--view",
      "--report",
      "--severity",
      "--type",
      "--timestamp",
      "--region",
      "--currency",
      "--no-time-series",
      "--no-save-report",
      "--wait",
      "--poll-interval",
      "--output",
      "--open",
      "--print-url",
      "--no-open",
      "--frontend-url",
      "--profile",
      "--help",
    ],
    workflows: [
      "reports list",
      "reports show",
      "reports cost",
      "reports waf",
      "reports download",
      "reports run",
      "reports rules",
    ],
  },
  {
    name: "projects",
    description: "Project utilities",
    domain: "projects",
    options: [
      "list",
      "get",
      "open",
      "create",
      "--template-url",
      "--template-file",
      "--parameters-file",
      "--parameters-url",
      "--name",
      "--description",
      "--provider",
      ...outputOptions,
      ...authOptions,
      "--profile",
      "--help",
    ],
    workflows: ["projects list", "projects get", "projects open", "projects create"],
  },
  {
    name: "connections",
    description: "Connection utilities",
    domain: "connections",
    options: ["list", "get", "open", ...outputOptions, ...authOptions, "--profile", "--help"],
    workflows: ["connections list", "connections get", "connections open"],
  },
  {
    name: "billing",
    description: "Billing and usage utilities",
    domain: "billing",
    options: [
      "summary",
      "plans",
      "usage",
      "ledger",
      "invoices",
      "topups",
      "notifications",
      "--range",
      "--start-at",
      "--end-at",
      "--granularity",
      "--action-type",
      "--model",
      "--outcome",
      "--charge-status",
      "--limit",
      "--cursor",
      ...outputOptions,
      ...authOptions,
      "--profile",
      "--help",
    ],
    workflows: [
      "billing summary",
      "billing usage",
      "billing ledger",
      "billing invoices",
      "billing topups",
      "billing plans",
      "billing notifications",
    ],
  },
  {
    name: "credits",
    description: "Show current credit stats",
    domain: "billing",
    options: [...outputOptions, ...authOptions, "--profile", "--help"],
    workflows: ["credits"],
  },
  {
    name: "open",
    description: "Open CloudEval frontend deeplinks",
    domain: "frontend",
    options: [
      "overview",
      "chat",
      "projects",
      "project",
      "connections",
      "connection",
      "reports",
      "billing",
      "--base-url",
      "--frontend-url",
      "--print-url",
      "--no-open",
      "--thread",
      "--project",
      "--tab",
      "--view",
      "--layout",
      "--node",
      "--resource",
      "--file",
      "--files",
      "--cursor",
      "--selection",
      "--workspace-focus",
      "--presentation",
      "--profile",
      "--help",
    ],
    workflows: [
      "open overview",
      "open chat",
      "open projects",
      "open project",
      "open connections",
      "open connection",
      "open reports",
      "open billing",
    ],
  },
  {
    name: "capabilities",
    description: "Show machine-readable CloudEval CLI capabilities",
    domain: "diagnostics",
    options: ["--format", "--profile", "--help"],
    workflows: ["capabilities"],
  },
  {
    name: "login",
    description: "Authenticate with CloudEval",
    domain: "config",
    options: ["--base-url", "--headless", "--verbose", "--profile", "--help"],
    workflows: ["login"],
  },
  {
    name: "logout",
    description: "Log out and clear stored authentication state",
    domain: "config",
    options: ["--base-url", "--all-devices", "--profile", "--help"],
    workflows: ["logout"],
  },
  {
    name: "auth",
    description: "Authentication utilities",
    domain: "config",
    options: ["status", "--base-url", "--profile", "--help"],
    workflows: ["auth status"],
  },
  {
    name: "banner",
    description: "Preview the startup banner and terminal capabilities",
    domain: "diagnostics",
    options: ["--profile", "--help"],
    workflows: ["banner"],
  },
  {
    name: "completion",
    description: "Print shell completion script",
    domain: "diagnostics",
    options: ["bash", "zsh", "fish", "--bin", "--profile", "--help"],
    workflows: ["completion"],
  },
  {
    name: "help",
    description: "Display help for humans or agents",
    domain: "diagnostics",
    options: ["agents", "--profile", "--help"],
    workflows: ["help", "help agents"],
  },
  {
    name: "config",
    description: "View and edit local CloudEval CLI settings",
    domain: "config",
    options: [
      "show",
      "get",
      "set",
      "unset",
      "path",
      "profiles",
      "--profile",
      "--format",
      "--output",
      "--help",
    ],
    workflows: ["config show", "config get", "config set", "config unset", "config path", "config profiles"],
  },
  {
    name: "setup",
    description: "Configure CloudEval CLI defaults",
    domain: "setup",
    options: [
      "--non-interactive",
      "--base-url",
      "--frontend-url",
      "--project",
      "--model",
      "--profile",
      "--format",
      "--output",
      "--help",
    ],
    workflows: ["setup"],
  },
  {
    name: "doctor",
    description: "Diagnose local CLI configuration and environment",
    domain: "diagnostics",
    options: ["--base-url", "--format", "--deep", "--profile", "--help"],
    workflows: ["doctor"],
  },
  {
    name: "status",
    description: "Show CloudEval CLI status",
    domain: "diagnostics",
    options: ["--base-url", "--format", "--profile", "--help"],
    workflows: ["status"],
  },
  {
    name: "models",
    description: "List models and manage the default model",
    domain: "models",
    options: [
      "list",
      "default",
      "get",
      "set",
      ...authOptions,
      "--format",
      "--output",
      "--profile",
      "--help",
    ],
    workflows: ["models list", "models default get", "models default set"],
  },
  {
    name: "sessions",
    description: "Manage local CLI session history",
    domain: "sessions",
    options: [
      "list",
      "get",
      "export",
      "delete",
      "prune",
      "--limit",
      "--output",
      "--format",
      "--yes",
      "--profile",
      "--help",
    ],
    workflows: ["sessions list", "sessions get", "sessions export", "sessions delete", "sessions prune"],
  },
];

export const commandNames = cliCommands.map((command) => command.name);

export const buildDomains = (): Record<CliDomain, string[]> => {
  const domains = {} as Record<CliDomain, string[]>;
  for (const command of cliCommands) {
    domains[command.domain] = [
      ...(domains[command.domain] ?? []),
      ...(command.workflows ?? [command.name]),
    ];
  }
  return domains;
};
